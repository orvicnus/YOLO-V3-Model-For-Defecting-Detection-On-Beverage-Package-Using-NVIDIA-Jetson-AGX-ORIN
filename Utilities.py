    ...
    for batch_idx, (x, labels) in enumerate(tqdm(loader)):
        ...
        for idx in range(batch_size):
            ...
            for nms_box in nms_boxes:
                all_pred_boxes.append([train_idx] + nms_box.tolist())  # Perbaikan di sini

            for box in true_bboxes[idx]:
                if box[1] > threshold:
                    all_true_boxes.append([train_idx] + box.tolist())  # Perbaikan di sini
...
def check_class_accuracy(model, loader, threshold):
    ...
    for idx, (x, y) in enumerate(tqdm(loader)):
        ...
        for i in range(3):
            ...
            # For object classification accuracy
            predicted_classes = torch.argmax(out[i][..., 5:], dim=-1)
            true_classes = y[i][..., 5]
            correct_class = (predicted_classes == true_classes)

            for c in range(NUM_CLASSES):
                class_correct[c] += torch.sum(correct_class & (true_classes == c)).item()
                class_total[c] += torch.sum(true_classes == c).item()
    ...
    for c in range(NUM_CLASSES):
        accuracy = (class_correct[c] / (class_total[c] + 1e-16)) * 100
        print(f"Accuracy for Class {c}: {accuracy:.2f}%")

    print(f"Class accuracy is: {(sum(class_correct)/(sum(class_total)+1e-16))*100:2f}%")
